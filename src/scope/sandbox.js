 ...
 */
 export function effect (microWindow) {
  ...
  microWindow.addEventListener = function (type, listener, options) {
    console.log('micro sub app ',type,listener)
...
export default class SandBox {
  ...
  constructor (app) {
    ...
    const proxyDoc = new Proxy(document,{
      get: (target, key) => {
         // 优先从代理对象上取值
        if (Reflect.has(this.shadow, key)) {
          let value = Reflect.get(this.shadow, key);
          if (typeof value === 'function') {
            // 如果值是函数，则需要绑定到window对象上
            return value.bind(this.shadow);
          }
          return value;
        }
    ...
    this.microWindow = {document:proxyDoc};
  ...
  // 绑定js作用域
  bindScope (code) {
    // const testCode = 'console.log("gg:",window,,window.document.querySelector,window.document.createElement)'
