 ...
 */
export default function scopedCSS (styleCode, appName) {
  let prefixStyle = "";
  // 前缀
  const prefix = `micro-demo[name=${appName}]`
  ...
  if (styleCode) {
    // 将元素的内容赋值给模版元素
    templateStyle.textContent = styleCode
    // 格式化规则，并将格式化后的规则赋值给style元素
    prefixStyle = scopedRule(Array.from(templateStyle.sheet?.cssRules ?? []), prefix)
    // 清空模版style内容
